...
void SHOW_HIGHSCORE(char path[])
{
    ...
    for (i = 0; i < 5; i++)
    {
        ...
        render(Image, serial_rect.w, serial_rect.h, serial_rect.x, serial_rect.y, NULL);

        name_rect.y += 50;
        name_rect.w = strlen(name) * 20;
        Image = scoreprint(name);
        render(Image, name_rect.w, name_rect.h, name_rect.x, name_rect.y, NULL);

        score_rect.y += 50;
        score_rect.w = strlen(score) * 20;
        Image = scoreprint(score);
        render(Image, score_rect.w, score_rect.h, score_rect.x, score_rect.y, NULL);
